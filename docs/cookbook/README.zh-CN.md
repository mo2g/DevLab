# AI Cookbook

[English](./README.md)

这部分记录我在 AI 项目中更关注的方法、取舍和工程经验，而不是只罗列功能点。大多数文章会围绕 `问题背景`、`设计决策`、`踩坑`、`权衡`、`可复用经验`、`下一步` 展开；当“思考过程”本身同样重要时，也会额外加入时间线或决策矩阵。每个课题都有自己的目录，目录内放置英文 `README.md` 和中文 `README.zh-CN.md`。

## 目录

- [我如何思考 Codex 工作流自动化](./codex-workflow-automation-tradeoffs/README.zh-CN.md)
- [Codex Composer 开发经验](./codex-composer-workflow/README.zh-CN.md)
- [Token Insight 开发经验](./token-insight-local-first-analytics/README.zh-CN.md)
- [AgentScan 工程思考](./agentscan-agent-security-notes/README.zh-CN.md)

## 阅读建议

- 如果你关心自动化程度、thread-first 和成本取舍，先看 [我如何思考 Codex 工作流自动化](./codex-workflow-automation-tradeoffs/README.zh-CN.md)
- 如果你关心 AI coding workflow，先看 [Codex Composer 开发经验](./codex-composer-workflow/README.zh-CN.md)
- 如果你关心本地优先可观测性，先看 [Token Insight 开发经验](./token-insight-local-first-analytics/README.zh-CN.md)
- 如果你关心 AI Agent 安全治理，先看 [AgentScan 工程思考](./agentscan-agent-security-notes/README.zh-CN.md)

## 相关入口

- [文档总览](../README.zh-CN.md)
- [AI 项目索引](../ai/README.zh-CN.md)
- [仓库中文首页](../../README.zh-CN.md)
